if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(position => {
        ...
        const fetchSpaces = function (latitude, longitude, radius) {
            return new Promise(function (resolve, reject) {
                resolve(
                    fetch(`/laravel_lokasi/public/api/spaces?lat=${latitude}&lng=${longitude}&rad=${radius}`)
                    .then((res) => res.json())
                    .then(function (data) {
        ...
        if (window.action == 'browse') {
            map.addEventListener('dragend', function (ev) {
                ...
                init(resultCoord.lat, resultCoord.lng, 1000)
            }, false);

            init(objLocalCoord.lat, objLocalCoord.lng, 1000)
        }
    ...
    //membuka link direction route maps
    function openDirection(lat, lng, id) {
        window.open(`/laravel_lokasi/public/space/${id}?from=${objLocalCoord.lat},${objLocalCoord.lng}&to=${lat},${lng}`, "_self");
    }
